...
etf_invested_amounts = etf_invested_amounts - BROKER_BUYING_FEE
...
print(f"yearly_cash_flow stat: {yearly_cash_flow_summary.iloc[0]}")
print(f"re_etf_stat stat: {re_summary.iloc[-1]}")
print(f"cumulated positive cash flow: {yearly_cash_flow.where(yearly_cash_flow > 0, 0).cumsum().T.describe().T.iloc[-1]}")
print(f"re_summary stat: {re_summary.iloc[-1]}")
print(f"re_ptf_returns stat: {re_ptf_returns.describe()}")
print(f"etf_summary stat: {etf_summary.iloc[-1]}")
print(f"etf_ptf_returns stat: {etf_ptf_returns.describe(percentiles=PLOT_PERCENTILES)}")
low_val = 1
high_val = 20
...
print(f"ETF > RE median: {len(etf_ptf_returns[etf_ptf_returns > re_ptf_returns.median()])/N_SAMPLE}")
# %
